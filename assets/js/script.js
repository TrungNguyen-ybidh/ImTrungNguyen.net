...
// ===== THEME-AWARE COLORS =====
// Read CSS custom properties so particles re-tint when the user toggles theme.
...
let accentRGB = cssVarRGB('--accent', '201,240,107');
let textRGB = cssVarRGB('--text-primary', '232,230,225');
window.addEventListener('themeChange', () => {
  accentRGB = cssVarRGB('--accent', '201,240,107');
  textRGB = cssVarRGB('--text-primary', '232,230,225');
});
...
class ShootingStar {
  ...
  draw() {
    ...
    g.addColorStop(0.7, `rgba(${accentRGB},${o * 0.4})`);
...
class Dust {
  ...
  draw() {
    ...
    ctx.fillStyle = `rgba(${accentRGB},${o})`;
    ctx.beginPath();
